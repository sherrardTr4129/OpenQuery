...
class CSHLDAP:
    def __init__(self, user, password, host='ldaps://ldap.csh.rit.edu:636', \
            base='ou=Users,dc=csh,dc=rit,dc=edu', bind='ou=Users,dc=csh,dc=rit,dc=edu', app = True):
        self.host = host
        self.base = base
        ...
        if app:
            print('uid=' + user + ',' + bind, password)
            self.ldap.simple_bind_s('uid=' + user + ',' + bind, password)
        else:
            try:
                auth = sasl.gssapi("")
